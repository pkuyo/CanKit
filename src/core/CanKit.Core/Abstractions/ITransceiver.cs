...
namespace CanKit.Core.Abstractions
{
    ...
    public interface ITransceiver
    {
        ...
        /// </returns>
        int Transmit(ICanBus<IBusRTOptionsConfigurator> bus,
        ...
        /// <summary>
        /// Sends one CAN frame via the specified channel. （通过指定通道发送一个 CAN 帧）
        /// </summary>
        /// <param name="bus">The channel on which to transmit. （执行发送操作的通道）</param>
        /// <param name="frames">The frames to transmit. （需要发送的帧集合）</param>
        /// <returns>
        ...
        /// （成功写入硬件的帧数量；若发生超时，可能小于请求数量。）
        /// </returns>
        int Transmit(ICanBus<IBusRTOptionsConfigurator> bus, in ICanFrame frames);
