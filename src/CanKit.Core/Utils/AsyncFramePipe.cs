...
public sealed class AsyncFramePipe
{
        ...
        uint count, int timeoutMs, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                // 同时等待：下一条消息 or 本次 fault 脉冲
                var readTask = _channel.Reader.ReadAsync(token).AsTask();
                var faultSnap = _exceptionPulse; // 捕获“本轮”的 TCS

                var completed = await Task.WhenAny(readTask, faultSnap.Task).ConfigureAwait(false);
