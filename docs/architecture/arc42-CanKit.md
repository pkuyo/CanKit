...
| L4 Anwendungsprotokolle | UDS-MVP vorhanden (`CanKit.Pro.Uds`); CANopen-MVP vorhanden (`CanKit.Pro.CANopen`); J1939-App-MVP vorhanden (`CanKit.Pro.J1939`, FR-J1939-001..006 Must, FR-J1939-007 Should — Single-Frame ≤ 8 Byte über L1 `IPeriodicTx` wenn der Adapter Bus-native zyklische TX liefert, andernfalls über die `SendAsync`-Schleife des J1939-Actors (identisch zum Multi-Frame-Pfad), damit `Transmit`-Fehler in beiden Pfaden über `BackgroundExceptionOccurred` sichtbar werden; Multi-Frame > 8 Byte grundsätzlich via Software-Schleife über J1939-TP); HAWE-Rahmen vorhanden (`CanKit.Pro.Hawe`) | Diagnose-/Applikationssemantik auf L3/L2. | `IUdsClient`/`UdsClient` (UDS); `ICanOpenNode`/`CanOpen`/`ObjectDictionary` (CANopen); `IJ1939Node`/`J1939Node` (J1939-App); HAWE-Rahmen: `IHaweCodec`/`IHaweCodecRegistry`/`HaweChannel` (generisch, kein Protokolldetail; CON-006) | `FR-UDS-*`, `FR-CO-*`, `FR-J1939-*`, `FR-HAWE-*` |
