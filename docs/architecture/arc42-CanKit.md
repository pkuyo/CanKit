...
| **L4** | Anwendungsprotokoll-Ebene | **MVP** (UDS `CanKit.Pro.Uds`, CANopen `CanKit.Pro.CANopen`, J1939-App `CanKit.Pro.J1939`; HAWE-Rahmen vorhanden, generisch – kein Protokolldetail; CON-006) | UDS, CANopen, J1939-App, HAWE-Privatprotokoll |
...
| `AsyncFramePipe<T>` | Entkopplung RX-Producer (Loop) ↔ async-Consumer. | `Publish`, `ReceiveBatchAsync`, `ReadAllAsync`, `ExceptionOccurred`. | `FR-RAW-ASYNC-*` |
...
flowchart TB
    ...
    FailA -->|ja| FailAsync["failAsyncReceivers(ex)<br/>→ AsyncFramePipe.ExceptionOccurred"]
...
Software-Gate aus (p99 ≤ 5,0 ms), um Shared-Runner-Ausreißer nicht als Hardware-
Plattformversagen zu bewerten.
...
| QS-3 | Echtzeit | Periodisches Software-TX mit Periode ≥ 1 ms über ≥ 500 Perioden auf Windows- und Linux-Referenzhosts; synthetische CI-Messung nutzt Virtual bei 5 ms Periode. | Softwarepfad: p99 absoluter Inter-Sende-Jitter ≤ 1,0 ms auf Referenzhosts; CI-Soft-Gate p99 ≤ 5,0 ms mit Jitter-Histogramm; keine Busy-Loop-CPU-Last. | NFR-RT-1 |
...
| W | ✅ *Behoben.* **`CanEndpoint.Parse` lowercased Host**: `zlg://USBCANFD-200U` → `usbcanfd-200u`; Sonderzeichen werfen. | Adapter müssen case-insensitiv sein (nicht garantiert); Namen mit Leerzeichen scheitern. | `Uri`-basiertes Parsen durch einen manuellen Tokenizer ersetzt: Scheme wird lower-case normalisiert, Host/Path behalten ihre Groß-/Kleinschreibung, `-`/`_`/`.`/`~` sowie prozentkodierte Sequenzen (z.B. Leerzeichen) sind zulässig. Regressionstests: `tests/CanKit.Tests/TestCases/CanEndpointParseTests.cs`. | §2.5 |
