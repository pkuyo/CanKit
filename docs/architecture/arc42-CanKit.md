...
| W | ✅ *Behoben.* **AsyncFramePipe Fehlerpfade**: verwaister Reader konsumierte später Frames; Nutzer-Cancellation wurde geschluckt. | Frame-Verlust nach Hintergrundfehlern; inkonsistenter Cancellation-Kontrakt. | `ReceiveBatchAsync` wartet auf Lesebereitschaft statt auf ein verwaistes `ReadAsync`; der verlierende Wait wird abgebrochen. Nutzer-Cancellation propagiert, Timeouts liefern weiterhin die bis dahin gelesene Teilliste; `ExceptionOccured` bleibt ein Wake-only-Pulse für aktuell wartende Reader. | §2.2 |
