...
| NFR-012 | Zeitstempel in neu geschaffenen L2/L3/L4-Datenstrukturen MÜSSEN einheitlich UTC verwenden (Korrektur der Inkonsistenz `DateTime.Now` vs. `DateTime.UtcNow` in L1). **Status:** addressed for touched paths — `CanReceiveData.SystemTimestamp` defaults to `DateTime.UtcNow`, and all adapter diagnostics/hot paths (ZLG, Kvaser, PCAN, ControlCAN, SocketCAN fallback) now emit UTC; asserted by `SystemTimestampUtcTests` against the Virtual adapter. | Should | Codereview-Checkliste; Unit-Test auf `DateTimeKind.Utc`. | Review §2.5 |
