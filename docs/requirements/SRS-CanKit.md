...
| FR-TP-002 | Das System MUSS eingehende ISO-TP-PDUs korrekt aus Single-, First- und Consecutive-Frames zusammensetzen und über `DatagramReceived`/`GetFramesAsync` bereitstellen. | Must | Integrationstest: Multi-Frame-Empfang mit korrekter Reassemblierung inkl. Sequenznummern-Prüfung. | ISO 15765-2 §6; Review §1.1 Punkt 7 (SN-Fehler als Negativbeispiel) |
...
Ist-Zustand: MVP umgesetzt als eigenständiges Paket `CanKit.Pro.J1939` (`src/protocols/CanKit.Pro.J1939`, `IsPackable=false`, Version 0.1.0). Actor-getriebener Node (`IJ1939Node`) auf Basis der L2-Dienste (`ICanBusService`, `IProtocolActor`, `IDeadlineScheduler`) sowie der J1939-Adressierungshelfer aus `CanKit.Pro.Addressing`; für Nutzlasten > 8 Byte wird automatisch die J1939-TP-Ebene (`CanKit.Pro.J1939Tp`) verwendet. Deckt FR-J1939-001..006 (Must) sowie FR-J1939-007 (Should) mittels Virtual-Loopback-Integrationstests in `tests/CanKit.Tests/TestCases/J1939/J1939NodeTests.cs` ab. FR-J1939-007 bevorzugt für Single-Frame-PGNs (≤ 8 Byte) die L1-Periodik `ICanBus.TransmitPeriodic` / `IPeriodicTx` via `ICanBusService.Bus`, sofern der Adapter Bus-native zyklische TX liefert. Wenn `TransmitPeriodic` stattdessen den `SoftwarePeriodicTx`-Fallback zurückgibt (Virtual sowie Adapter, die zyklische TX nur per Software-Fallback anbieten), verschluckt dessen `TrySendOnce` Sende-Ausnahmen und kann sie nicht über `IPeriodicTx` melden — der Node erkennt diesen Fall zur Startzeit und treibt die Periode dann über die `SendAsync`-Schleife des Actors (identisch zum Multi-Frame-Pfad), damit Fehler einheitlich über `BackgroundExceptionOccurred` sichtbar werden. Multi-Frame-PGNs (> 8 Byte) behalten die Software-Schleife, weil pro Emission eine eigene J1939-TP-Session (TP.BAM/TP.CM) aufgesetzt werden muss. Weiterhin offen: Feinabstimmung der SAE J1939-81 Arbitrationszeiten gegen reale Hardware sowie erweiterte Multi-Adress-Reservierungen.
...
| **Performance-/Timing-Test** | Messung von Jitter, Durchsatz, STmin-Genauigkeit, CPU-Auslastung im Leerlauf. | NFR-001..003, NFR-007 | Referenzplattformen (Windows, Linux; macOS für NFR-002). |
...
| FR-J1939-001..007 | L4 – *J1939-Applikationsschicht* (`src/protocols/CanKit.Pro.J1939`, MVP; FR-J1939-001..006 Must umgesetzt, FR-J1939-007 Should — Single-Frame ≤ 8 Byte über L1 `IPeriodicTx` bei nativer zyklischer TX, sonst über die `SendAsync`-Actor-Schleife; Multi-Frame > 8 Byte grundsätzlich über J1939-TP mit Software-Schleife — beide Pfade melden `Transmit`-Fehler über `BackgroundExceptionOccurred`) | Virtual-Loopback-Integrationstest (`tests/CanKit.Tests/TestCases/J1939/J1939NodeTests.cs`), HIL-Stichprobe |
